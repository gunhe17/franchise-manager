...
class PointType(ValueObject):
    _value: str

    # hint
    _allowed_list: tuple[str, ...] = ("order", "return", "adjustment", "promotion")

    # #
    # factory

    @classmethod
    def from_str(cls, value) -> "PointType":
        # type
        if not isinstance(value, str):
            raise InvalidError("PointType")

        # format
        if value not in cls._allowed_list:
            raise InvalidFormatError("PointType")
