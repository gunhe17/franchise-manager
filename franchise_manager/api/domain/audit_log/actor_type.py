...
class ActorType(ValueObject):
    _value: str

    # hint
    _allowed_list: tuple[str, ...] = ("admin", "user", "system")

    # #
    # factory

    @classmethod
    def from_str(cls, value) -> "ActorType":
        # type
        if not isinstance(value, str):
            raise InvalidError("ActorType")

        # format
        if value not in cls._allowed_list:
            raise InvalidFormatError("ActorType")
